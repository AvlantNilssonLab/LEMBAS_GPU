...
from ..model.model_utilities import np_to_torch, format_network
from ..model.activation_functions import activation_function_map
...
def spectralLoss(signalingModel, YhatFull, weights, expFactor = 20, lb=0.5):
    ...
    #x_prime = self.onestepdelta_activation_factor(YhatFull[randomIndex,:], self.training_params['leak'])     
    #x_prime = x_prime.unsqueeze(2)
    #T = x_prime * self.weights



    bionetParams = signalingModel.training_params

    randomIndex = np.random.randint(YhatFull.shape[0])
    activationFactor = signalingModel.onestepdelta_activation_factor(YhatFull[randomIndex,:], signalingModel.training_params['leak']).detach()
    weightFactor = activationFactor[signalingModel.edge_list[0]] # networklist <--> edgelist


    # Create sparse matrix using COO format
    sparse_matrix = (scipy.sparse.coo_matrix((weightFactor.cpu(), (signalingModel.edge_list[0], signalingModel.edge_list[1])), shape=weights.shape))
    # Convert the sparse matrix to a dense (NumPy) matrix
    dense_matrix = sparse_matrix.toarray()
    # Convert the dense NumPy matrix to a PyTorch tensor
    torch_dense_tensor = torch.tensor(dense_matrix, dtype=torch.float32)

    multipliedWeightFactor = weights.cpu() * (torch_dense_tensor)
    ...
    # else:
    #     spectralRadiusLoss = torch.tensor(0.0)s

    
    #spectralRadiusLoss = scaleFactor * (torch.exp(expFactor*spectralRadius)-1)
    if spectralRadius>lb:
        #spectralRadiusLoss =  (torch.exp(signalingModel.training_params['exp_factor']* (spectralRadius-signalingModel.training_params['spectral_target'])))
        scaleFactor = 1/np.exp(signalingModel.training_params['exp_factor'] *  signalingModel.training_params['spectral_target'])
        spectralRadiusLoss = scaleFactor * (torch.exp(signalingModel.training_params['exp_factor']*spectralRadius)-1)
    else:
...
class spectralRadius(torch.autograd.Function):
    ...
    def forward(ctx, weights, A, networkList):
        ...
        ctx.tol = 10**-6 # Change TOO 5 as other use 6 but than mults with 10
        weights = weights.detach().numpy()

        ctx.networkList = networkList
        ctx.weights = weights
        ctx.A = A
        ctx.A.data = ctx.weights[ctx.weights.nonzero()]
        ...
        return torch.from_numpy(np.asarray(spectralRadius))
    ...
    def backward(ctx, grad_output):
        ...
        tmpA = ctx.A
        tmpA.data = ctx.weights[ctx.weights.nonzero()]
        ...
        # Create sparse matrix using COO format
        dW = (scipy.sparse.coo_matrix((dW.cpu(), (networkList[0], networkList[1])), shape=ctx.weights.shape))
        # Convert the sparse matrix to a dense (NumPy) matrix
        dW = dW.toarray()
        # Convert the dense NumPy matrix to a PyTorch tensor
...
class BioNet(nn.Module):
    ...
    def __init__(self, edge_list: np.array, 
        ...
        self.A=scipy.sparse.csr_matrix(weights.cpu().detach().numpy(),  shape=(size, size), dtype='float64')
    ...
    def L2_reg_zero(self, lambda_L2: Annotated[float, Ge(0)] = 0):
        """Get the L2 regularization term for the neural network parameters.
        ...
        bionet_L2_zero = lambda_L2 * torch.sum(1/(self.weights**2 + 0.5)) + self.L2_reg(lambda_L2)
    ...
    def getSpectralRadius(self, weights):
        ...
        return spectralRadius.apply(weights, self.A, self.edge_list)
    ...
    def L2_reg_zero(self, lambda_L2: Annotated[float, Ge(0)] = 0):
        ...
        -------
         : torch.Tensor
            the regularization term (as the sum of the regularization terms for each layer)
        """
        return self.input_layer.L2_reg(lambda_L2) + self.signaling_network.L2_reg(lambda_L2) + self.output_layer.L2_reg(lambda_L2)+self.signaling_network.L2_reg_zero(lambda_L2)
