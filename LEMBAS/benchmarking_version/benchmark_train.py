...
#     else:
#         if dict_to_bench['ProjectOutput_bias']==0:
#             L2_reg=mod.deprecation_L2_reg_no_bias_zero
#         else:
#             L2_reg=mod.deprecation_L2_reg_zero # zero here means that we penelize zero through 1/=0.5=w**2 and remove gradient noise
    ...
    for e in tqdm(range(hyper_params['max_iter']),'training'):
        # set learning rate
        add_noise=(e<hyper_params['max_iter']*0.9)
        cur_lr = utils.get_lr(e, hyper_params['max_iter'], max_height = hyper_params['learning_rate'],
                              start_height=hyper_params['learning_rate']/10, end_height=1e-5, peak = 1000)
        ...
        for batch, (X_in_, y_out_) in enumerate(train_dataloader):
            ...
            X_full = X_full + (hyper_params['noise_level'] * cur_lr * network_noise)*add_noise # randomly add noise to signaling network input, makes model more robust
            ...
            total_loss = fit_loss + sign_reg + ligand_reg + param_reg  + uniform_reg+ stability_loss

            total_loss.backward()

            if dict_to_bench["L2_or_rand"]==0:# If we use normal L2 regulization than we should add gradient noise.
                
                mod.add_gradient_noise(noise_level =  (cur_lr**2) * hyper_params['gradient_noise_level'],add_this=add_noise)
