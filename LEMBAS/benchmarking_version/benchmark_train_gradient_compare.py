...
from LEMBAS.benchmarking_version.benchmark_bionetwork_gradient_compare import spectralLoss

LR_PARAMS = {'max_iter': 500, 'learning_rate': 2e-3}
...
def train_signaling_model(mod,  
    ...
    iteration=hyper_params['max_iter']
    correlator=[]
    q=5
    mod.signaling_network.force_sparcity()
    for e in tqdm.tqdm(range(iteration)):
        ...
        cur_eig = []
        if (e%q)==(q-1):
            mod_old_eigen = copy.deepcopy(mod)# For the last iteration save a deepcopy of the model to be used for a training step with the other loss
            optimizer = optimizer_choosen(mod.parameters(), lr=1, weight_decay=0)# For last step reset optimzer to have comparable results as adam saves and scales things
            cur_lr = utils.get_lr(e, hyper_params['max_iter'], max_height = hyper_params['learning_rate'],
                                start_height=hyper_params['learning_rate']/10, end_height=1e-6, peak = 1000)
            optimizer.param_groups[0]['lr'] = cur_lr
        ...
        for batch, (X_in_, y_out_) in enumerate(train_dataloader):
            mod.train()
            optimizer.zero_grad()
            #print(mod.signaling_network.weights.detach().cpu().numpy().flatten())
            ...
            if (e%q)==(q-1):
                #{"name": "steady_state_new_poweriter_", "uniform":0,"L2_or_rand" :0,"stability_eigen":1,"device": "cuda"}
                weights_before_steps=mod.signaling_network.weights.detach().cpu().numpy()
                if dict_to_bench["test_eigen_1_or_uni_0"]==1:
                
                    total_loss = stability_loss
                    print('pi')
                    print(total_loss)
                elif dict_to_bench["test_eigen_1_or_uni_0"]==0:
                    total_loss=uniform_reg
                else:
                    pass
            ...
            gradients_1 = {name: param.grad for name, param in mod.named_parameters() if param.grad is not None}
            ...
            # store
            cur_eig.append(spectral_radius)#.item())
        ...
        if e%(q)==q-1:
            mod_old_eigen = copy.deepcopy(mod)# For the last iteration save a deepcopy of the model to be used for a training step with the other loss
            # Now we can use the saved model and train if for a final iteration step and update its parameters to have a model that we can compare our final model too.
            optimizer = optimizer_choosen(mod_old_eigen.parameters(), lr=1, weight_decay=0)
            reset_state = optimizer.state.copy()
            optimizer.param_groups[0]['lr'] = cur_lr


            X_in_, y_out_ = X_in_.to(mod_old_eigen.device), y_out_.to(mod_old_eigen.device) # We still have access to this batch
            
            # forward pass
            X_full = mod_old_eigen.input_layer(X_in_) # transform to full network with ligand input concentrations
            utils.set_seeds(mod_old_eigen.seed + mod_old_eigen._gradient_seed_counter)

            X_full = X_full + (hyper_params['noise_level'] * cur_lr * network_noise) # randomly add noise to signaling network input, makes mod_old_eigenel more robust
            Y_full = mod_old_eigen.signaling_network(X_full) # train signaling network weights
            Y_hat = mod_old_eigen.output_layer(Y_full)
            ...
            if dict_to_bench['test_eigen_1_or_uni_0']==1:
                spectralRadiusLoss, spectralRadius = spectralLoss(mod_old_eigen.signaling_network, Y_full.detach(), mod_old_eigen.signaling_network.weights, expFactor = 50)
                spectralRadiusLoss = spectralRadiusLoss * hyper_params['spectral_loss_factor']

                total_loss = spectralRadiusLoss
                print('eigs')
                print(total_loss)
            elif dict_to_bench["test_eigen_1_or_uni_0"]==0:
                total_loss=mod.deprecation_parametric_uniform_regularization(lambda_L2 = hyper_params['uniform_lambda_L2']*cur_lr, Y_full = Y_full, 
                                                     target_min = 0, target_max = hyper_params['uniform_max']) # uniform distribution
            else:
                pass
            ...
            gradients_2 = {name: param.grad for name, param in mod_old_eigen.named_parameters() if param.grad is not None}



            gradients_1['signaling_network.weights']=gradients_1['signaling_network.weights'].cpu()* (weights_before_steps != 0).astype(float)
            gradients_2['signaling_network.weights']=gradients_2['signaling_network.weights'].cpu()
            norm_grad1 = torch.norm(gradients_1['signaling_network.weights'])
            norm_grad2 = torch.norm(gradients_2['signaling_network.weights'])
            print(f"Norm of gradients 1: {norm_grad1.item()}, Norm of gradients 2: {norm_grad2.item()}")
            zero_mask = gradients_1['signaling_network.weights'] != 0.0
            zero_mask = zero_mask* gradients_2['signaling_network.weights'] != 0.0
            gradients_1 = gradients_1['signaling_network.weights'][zero_mask]
            gradients_2 = gradients_2['signaling_network.weights'][zero_mask]
            
            pr, _ = pearsonr(gradients_1, gradients_2)
            print(pr)
    ...
    return mod, mod_old_eigen, gradients_1,gradients_2,correlator