...
def train_signaling_model(mod,  
                          ...
                          train_split_frac: Dict = {'train': 1.0, 'test': 0.0, 'validation': None},
                          train_seed: int = None,
                         verbose: bool = True,
                          break_nan: bool = True, 
    ...
    # set up data objects
    # Check if the tensor is single or double precision
    ...
    if dict_to_bench["L2_or_rand"]==0:
        if dict_to_bench['ProjectOutput_bias']==0:
            L2_reg=mod.deprecation_L2_reg_no_bias
            mod.output_layer.bias.requires_grad = False
        else:
            L2_reg=mod.L2_reg
    else:
        if dict_to_bench['ProjectOutput_bias']==0:
            L2_reg=mod.deprecation_L2_reg_no_bias_zero # zero here means that we penelize zero through 1/=0.5=w**2 and remove gradient noise
            mod.output_layer.bias.requires_grad = False
        else:
            L2_reg=mod.deprecation_L2_reg_zero


    if dict_to_bench["uniform"]==0:
        Uni_Reg=mod.uniform_regularization
    else:
        Uni_Reg=mod.deprecation_parametric_uniform_regularization
    ...
    # begin iteration
    if dict_to_bench['ProjectOutput_bias'] == 0:
        final_output = mod.output_layer.deprecation_forward_no_bias
    else:
        final_output = mod.output_layer
    
    mod.signaling_network.force_sparcity()
    cur_eig = []
    for e in tqdm(range(hyper_params['max_iter']),'training'):
        # set learning rate

        cur_lr = utils.get_lr(e, hyper_params['max_iter'], max_height = hyper_params['learning_rate'],
                              start_height=hyper_params['learning_rate']/10, end_height=hyper_params['learning_rate']*1e-2, peak = 3000)
        ...
        for batch, (X_in_, y_out_) in enumerate(train_dataloader):
            ...
            Y_hat = final_output(Y_full) # transform to TF activity
            # get prediction loss
            ...
            total_loss = fit_loss + (sign_reg + ligand_reg+ L_2  + stability_loss + uniform_reg)
  
            total_loss.backward()
            # Before adding gradient noise
            #grad_norm_before = torch.norm(torch.stack([torch.norm(p.grad) for p in mod.parameters() if p.grad is not None]))


            # Add gradient noise if needed
            if dict_to_bench["L2_or_rand"] == 0:  # If we use normal L2 regularization
                mod.add_gradient_noise(noise_level= cur_lr* hyper_params['gradient_noise_level'])#cur_lr *
            
            # After adding gradient noise
            #grad_norm_after = torch.norm(torch.stack([torch.norm(p.grad) for p in mod.parameters() if p.grad is not None]))

            # Clip gradients (L2 norm clipping, for example at 1.0)
            torch.nn.utils.clip_grad_norm_(mod.parameters(), max_norm=1.0)

            #grad_norm_after_clipping = torch.norm(torch.stack([torch.norm(p.grad) for p in mod.parameters() if p.grad is not None]))
            optimizer.step()
