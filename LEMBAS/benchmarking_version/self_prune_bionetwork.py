...
# Function to generate unique edges
def generate_unique_edges(existing_edges, num_nodes, num_new_edges):
    existing_edges_set = {tuple(edge) for edge in existing_edges.T}
    new_edges = []

    while len(new_edges) < num_new_edges:
        edge = tuple(np.random.randint(0, num_nodes, size=2))
        if edge not in existing_edges_set:
            existing_edges_set.add(edge)
            new_edges.append(edge)

    return np.array(new_edges).T  # Transpose to match (2, num_edges) shape
...
class BioNet(nn.Module):
    ...
    def __init__(self, edge_list: np.array, 
        ...
        add_random="random"#"bayesian"#,"random","all"
        # ADD NEW NODES
        
        self.edge_real = (np_to_torch(edge_list[0,:], dtype = torch.int32, device = 'cpu'), 
                          np_to_torch(edge_list[1,:], dtype = torch.int32, device = 'cpu'))
        edge_array = np.stack((edge_list[0, :], edge_list[1, :]), axis=1)
        edge_array = np.vstack((edge_list[0, :], edge_list[1, :]))
        np.savetxt("edges.txt", edge_array, delimiter=",", fmt="%d")
        if add_random=="random":


            # Define parameters

            edges_to_add = dict_to_bench["edges_to_add"]

            # Generate new edges
            new_edges = generate_unique_edges(edge_list, n_network_nodes, edges_to_add)

            # Append new edges
            edge_list = np.hstack((edge_list, new_edges))

            # Generate new MOA rows with correct shape (2, edges_to_add)
            new_MOA_rows = np.random.choice([False, False], size=(2, edges_to_add))  
            edge_MOA = np.hstack((edge_MOA, new_MOA_rows))  # Now shapes match
                        
        elif add_random=="bayesian":
            # Compute the degree of each node to use as weights
            node_degrees = np.bincount(edge_list.flatten(), minlength=n_network_nodes)
            node_weights = node_degrees / node_degrees.sum()

            # Convert edge_list to a set of tuples for fast lookup
            existing_edges = set(map(tuple, edge_list.T))

            # Generate unique new edges with preference for well-connected nodes
            new_edges = []
            while len(new_edges) < dict_to_bench["edges_to_add"]:
                # Sample two nodes with replacement based on weights
                source = np.random.choice(n_network_nodes, p=node_weights)
                target = np.random.choice(n_network_nodes, p=node_weights)

                # Ensure the new edge is not already in the existing edge list and is not a self-loop
                if (source != target) and ((source, target) not in existing_edges) and ((target, source) not in existing_edges):
                    ...
                    existing_edges.add((source, target))  # Add to the set to prevent duplicates

            # Convert new_edges list to numpy array
            new_edges = np.array(new_edges).T  # Shape (2, edges_to_add)

            # Append new edges to the edge list
            edge_list = np.hstack((edge_list, new_edges))

            # Generate random MOA rows for the new edges
            new_MOA_rows = np.random.choice([False, False], size=(2, dict_to_bench["edges_to_add"]))
            edge_MOA = np.hstack((edge_MOA, new_MOA_rows))

        else:
        # Generate all possible edges between the nodes
            all_possible_edges = np.array(
                [(i, j) for i in range(n_network_nodes) for j in range(n_network_nodes) ]
            ).T

            # Combine the new edges with the existing edge list
            edge_list = (( all_possible_edges))

            # Create new MOA rows for all added edges
            # Assuming you want False for new rows; modify if needed
            new_MOA_rows = np.zeros((2, all_possible_edges.shape[1]), dtype=bool)
            edge_MOA = np.hstack((edge_MOA, new_MOA_rows))

        self.random_edges=new_edges
    ...
    def forward_for_benching(self, X_full: torch.Tensor):
        """Learn the edeg weights within the signaling network topology.
    ...
    def get_SS_loss(self, Y_full: torch.Tensor, spectral_loss_factor: float, subset_n: int = 10, **kwargs):
        ...
        loss =  SS_deviation/torch.sum(SS_deviation.detach())
...
class SignalingModel(torch.nn.Module):
    ...
    def __init__(self, net: pd.DataFrame, X_in: pd.DataFrame, y_out: pd.DataFrame,
                ...
                bionet_params: Dict[str, float] = None , 
                 activation_function: str='MML', dtype: torch.dtype=torch.float32, device: str = 'cpu', seed: int = 888,dict_to_bench=0):
        ...
        self.signaling_network = BioNet(edge_list = edge_list, 
                                        ...
                                        dtype = self.dtype, device = self.device, seed = self.seed,dict_to_bench=dict_to_bench)
    ...
    def moments_based_uniform_regularization(self, lambda_L2: float, Y_full: torch.Tensor, 
                     target_min: float = 0.0, target_max: float = None):
        """Get the L2 regularization term for deviations of the nodes in Y_full from that of a uniform distribution between 
        `target_min` and `target_max`. 
        Note, this penalizes both deviations from the uniform distribution AND values that are out of range (like a double penalty).
        ...
        mean=(target_max-target_min)/2
        std=((target_max-target_min)**2)/12
        mean_Y=(torch.mean(Y_full,axis=1)-mean)**2
        std_Y=(torch.std(Y_full,axis=1)-std)**2

        moments_sum=torch.sum(mean_Y+std_Y)



        below_loss = torch.sum(Y_full.lt(target_min) * torch.square(Y_full-target_min)) # those that are below the minimum value
        above_loss = torch.sum(Y_full.gt(target_max) * torch.square(Y_full-target_max)) # those that are above the maximum value
        loss = lambda_L2*(moments_sum + below_loss + above_loss)
