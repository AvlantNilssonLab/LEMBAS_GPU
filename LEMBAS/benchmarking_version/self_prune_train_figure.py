...
def train_signaling_model(mod,  
    ...
    if mod.device == 'cuda':
        ...
        pin_memory = False
    folder = dict_to_bench["folder"]
    if "_syn_" in folder:
        fname = 'LEMBAS/benchmarking_version/ground_truth.txt'
    else:
        fname = 'LEMBAS/benchmarking_version/ground_truth_macrophage.txt'

    ground_indices = np.loadtxt(fname, delimiter=',', dtype=int)
    ...
    for e in tqdm.tqdm(range(0,hyper_params['max_iter'])):
        ...
        if e%q==0:
            ...
            #cutoff = np.percentile(combined_values, 5)
            #true_percentage = np.mean(true_values < cutoff) * 100
            #random_percentage = np.mean(random_values < cutoff) * 100
            #true_pecentage_list.append(true_percentage)
            #random_pecentage_list.append(random_percentage)
