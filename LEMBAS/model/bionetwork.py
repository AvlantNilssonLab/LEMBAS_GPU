...
class BioNet(nn.Module):
    ...
    x = torch.randn(5, requires_grad=True, device="cuda")

    # Hook function to remove specific gradient elements
    def mask_hook(self,grad):


        mask = self.weights
        new_grad = grad * mask  # Zero out gradients where mask is 0
    ...
    def forward(self, X_full: torch.Tensor):
        ...
        """
        #    hook = x.register_hook(mask_hook)
        ...
        Y_full = X_new.T
        return Y_full
    def forward_path(self, X_full: torch.Tensor):
        """Learn the edge weights within the signaling network topology and track the full evolution.
        ...
        Y_full : torch.Tensor
            The signaling network scaled by learned interaction weights. Shape is (samples x network nodes).
        path : torch.Tensor
            Tensor tracking the entire evolution over iterations. Shape is (time_steps x samples x network nodes).
        """
        X_bias = X_full.T + self.bias  # Bias with projection amplitude
        X_new = torch.zeros_like(X_bias)  # Initialize all values at 0
        max_steps = self.training_params['max_steps']

        # Preallocate tensor for storing evolution path
        path = torch.zeros((max_steps, *X_bias.shape), device=X_bias.device)
        
        for t in range(max_steps):  # Like an RNN, updating from the previous time step

            X_new = torch.mm(100*self.weights, X_new)  # Scale matrix by edge weights
            X_new = X_new + X_bias  # Add original values and bias
            X_new = self.activation(X_new, self.training_params['leak'])

            path[t] = X_new  # Store state


        Y_full = X_new.T
        return Y_full, path

    def forward_for_gpu_cpu_compare(self, X_full: torch.Tensor):
        """Learn the edeg weights within the signaling network topology.
        ...
        for t in range(100): # like an RNN, updating from previous time step
            X_old = X_new
            X_new = torch.mm(self.weights, X_new) # scale matrix by edge weights
            X_new = X_new + X_bias  # add original values and bias       
            X_new = self.activation(X_new, self.training_params['leak'])
...
class SignalingModel(torch.nn.Module):
    ...
    def deprecation_forward_no_bias(self, X_in):
        """Like forward, but with no bias term in the ProjectOutput layer."""
        ...
        Y_hat = self.output_layer.deprecation_forward_no_bias(Y_full) # TF outputs of signaling network
        return Y_hat, Y_full
    ...
    def forward(self, X_in):
        ...
        return Y_hat, Y_full
    def forward_for_gpu_cpu_compare(self, X_in):
        """Linearly scales ligand inputs, learns weights for signaling network interactions, and transforms this to TF activity. See
        `forward` methods of each layer for details."""
        X_full = self.input_layer(X_in) # input ligands to signaling network
        Y_full = self.signaling_network.forward_for_gpu_cpu_compare(X_full) # RNN of full signaling network
        Y_hat = self.output_layer(Y_full) # TF outputs of signaling network
        return Y_hat, Y_full
    def L2_reg(self, lambda_L2: Annotated[float, Ge(0)] = 0): #, 
    ...
    def deprecation_L2_reg_no_bias(self, lambda_L2: Annotated[float, Ge(0)] = 0):
        """Like L2_reg, but with no bias term in the ProjectOutput layer."""
        return self.input_layer.L2_reg(lambda_L2) + self.signaling_network.L2_reg(lambda_L2) + self.output_layer.deprecation_L2_reg_no_bias(lambda_L2)
    
    def deprecation_L2_reg_zero(self, lambda_L2: Annotated[float, Ge(0)] = 0):
        return self.input_layer.L2_reg(lambda_L2) + self.signaling_network.L2_reg(lambda_L2) + self.output_layer.L2_reg(lambda_L2)+self.signaling_network.deprecation_L2_reg_zero(lambda_L2)
    
    def deprecation_L2_reg_no_bias_zero(self, lambda_L2):
        return self.input_layer.L2_reg(lambda_L2) + self.signaling_network.L2_reg(lambda_L2) + self.output_layer.deprecation_L2_reg_no_bias(lambda_L2)+self.signaling_network.deprecation_L2_reg_zero(lambda_L2)
    ...
    def add_gradient_noise(self, noise_level: Union[float, int],add_this=True):
