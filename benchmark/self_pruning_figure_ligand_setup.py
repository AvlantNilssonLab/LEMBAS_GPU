...
from LEMBAS.benchmarking_version.self_prune_train_figure_ligand_setup import train_signaling_model
...
def benchmark_this_dict(dict_to_bench):
    ...
    # training parameters
    lr_params = {'max_iter': 16000, 
                'learning_rate': 1e-3}
    other_params = {'batch_size': 150, 'noise_level': 1.0, 'gradient_noise_level': 0.01}
    regularization_params = {'param_lambda_L2': 1e-6*(dict_to_bench["L2_norm"]), 'moa_lambda_L1': 0.1, 'ligand_lambda_L2': 1e-4, 'uniform_lambda_L2': 1e-5, 
...
for i in tqdm.tqdm(range(0,50)):
    dict_to_bench["num_epochs"]=16000
    ...
    dict_to_bench["L2_norm"]=1
    dict_to_bench["folder"]="parameter_study_ligand_"+str(dict_to_bench["L2_norm"])#macro,syn,ligands
...
# Save as CSV files
df_true_net.to_csv(f'{output_dir}/true_net_with_ligand_setup_self_prune.csv', index=False)
df_false_net.to_csv(f'{output_dir}/false_net_with_ligand_setup_self_prune.csv', index=False)
df_correlator_over_time_true.to_csv(f'{output_dir}/correlator_over_time_true_with_ligand_setup_self_prune.csv', index=False)
df_mean_test.to_csv(f'{output_dir}/mean_test_with_ligand_setup_self_prune.csv', index=False)
df_mean_train.to_csv(f'{output_dir}/mean_train_with_ligand_setup_self_prune.csv', index=False)
true_per_df.to_csv(f'{output_dir}/true_per_with_ligand_setup_self_prune.csv', index=False)
stocastic_per_df.to_csv(f'{output_dir}/stocastic_per_with_ligand_setup_self_prune.csv', index=False)
