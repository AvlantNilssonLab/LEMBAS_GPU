...
class model(torch.nn.Module):
    def __init__(self, networkList, nodeNames, modeOfAction, inputAmplitude, projectionAmplitude, inName, outName, bionetParams, activationFunction='MML', valType=torch.float32):
        ...
        self.projectionLayer = projectOutput(nodeNames, outName, projectionAmplitude, valType)
    ...
    def addNoiseToAllGradients(self, noiseLevel):
        allParams = list(self.parameters())
        for i in range(len(allParams)):
            if allParams[i].requires_grad:
                allParams[i].grad += noiseLevel * torch.randn(allParams[i].grad.shape, dtype=allParams[i].dtype, device=allParams[i].device)       
...
class projectInput(nn.Module):
    def __init__(self, nodeList, inputNames, projectionAmplitude, dtype):
        super().__init__()
        ...
        self.nodeOrder = torch.tensor(numpy.array([dictionary[x] for x in inputNames]))
        weights = projectionAmplitude * torch.ones(len(inputNames), dtype=dtype)
        self.weights = nn.Parameter(weights)
        self.projectionAmplitude = projectionAmplitude

    def forward(self, x):
        curIn = torch.zeros([x.shape[0],  self.size_out], dtype=x.dtype, device=x.device)
    ...
    def setDevice(self, device):
        self.nodeOrder = self.nodeOrder.to(device)


class projectOutput(nn.Module):
    def __init__(self, nodeList, outputNames, projectionAmplitude, type):
        super().__init__()
        ...
        dictionary = dict(zip(nodeList, list(range(len(nodeList)))))
        self.nodeOrder = torch.tensor(numpy.array([dictionary[x] for x in outputNames]))

        #bias = torch.zeros(len(outputNames), dtype=type)
        weights = projectionAmplitude * torch.ones(len(outputNames), dtype=type)
        self.weights = nn.Parameter(weights)
        self.projectionAmplitude = projectionAmplitude
...
class bionet(nn.Module):
    def __init__(self, networkList, size, modeOfAction, parameters, activationFunction, dtype):
        ...
        self.size_in = size
        self.size_out = size
        self.networkList = (torch.tensor(networkList[0, :]), torch.tensor(networkList[1, :]))
    ...
    def makeMOAMask(self):
        ...
        weights = torch.zeros(self.size_out, self.size_in, dtype=torch.long)
        weights[self.networkList] = MOAsigned
        MOAmask = weights == 0
        return weights, MOAmask

    def makeMask(self, networkList, size):
        weights = torch.zeros(size, size, dtype=bool)
    ...
    def setDevice(self, device):
        ...
        self.modeOfAction = self.modeOfAction.to(device)
        self.mask = self.mask.to(device)
        self.modeOfActionValues = self.modeOfActionValues.to(device)
        self.modeOfActionMask = self.modeOfActionMask.to(device)
        #self.step.setDevice(device)
        #self.networkList[0] = self.networkList[0].to(device)
        #self.networkList[1] = self.networkList[1].to(device)
    ...
    def signRegularization(self, MoAFactor):
        ...
        signMissmatch = torch.ne(torch.sign(self.weights), self.modeOfActionValues).type(self.weights.dtype)
    ...
    def initializeWeights(self):
        ...
        weights = 0.1 + 0.1 * torch.rand(size, dtype=self.type)
